...
print("--- SYSTEM: SAFE MODE STARTING ---") 
...
async def on_ready():
    ...
    try:
        ...
        await bot.change_presence(activity=discord.Activity(type=discord.ActivityType.watching, name="Procraft (Safe Mode) 🛡️"))
    except Exception as e:
        print(f"❌ ERROR SYNCING: {e}")

# ==========================================
#      🕹️ CHAOS PANEL (Now Safer!)
# ==========================================
...
class ChaosView(discord.ui.View):
    ...
    async def hello_spam(self, interaction: discord.Interaction, button: discord.ui.Button):
        await interaction.response.send_message("🚀 Spamming (Safely)...", ephemeral=True)
        # Slowed down to prevent Bans
        for i in range(5):
            await interaction.followup.send(f"Hello! 👋 (Message {i+1})", ephemeral=False)
            await asyncio.sleep(2) # <--- INCREASED WAIT TO 2 SECONDS
...
@app_commands.checks.cooldown(1, 10.0) # Only 1 use every 10 seconds
...
# ==========================================
#      🤡 FAKE PROMOTION (Cooldown Added)
# ==========================================

@bot.tree.command(name="promote", description="👮‍♂️ Promotes a user to Admin (FAKE).")
@app_commands.checks.cooldown(1, 5.0) # Safety Brake
...
# THE LISTENER (Added Error Handling to prevent crashes)
@bot.event
...
# --- GLOBAL ERROR HANDLER (This catches the cooldowns) ---
@bot.tree.error
